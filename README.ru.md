...
- Отдельная страница LuCI на английском и русском языках, доступная после обычного входа.
...
Войдите на роутер по SSH с правами root и выполните одну команду:
...
Эта же команда обновляет существующую установку, даже если запустить её из папки со старыми исходниками. Ошибка скачивания возвращает ненулевой код завершения. Установщик загружает четыре файла из соответствующего тега версии, готовит все замены и резервные копии до изменения рабочих файлов. При ошибке замены или регистрации RPC он пытается восстановить предыдущие файлы и сообщает, если восстановление не удалось.

Для ручной установки скачайте и распакуйте все исходники:
...
Для обновления используйте однострочную команду выше или скачайте свежий релиз и выполните `sh install.sh`. Существующие файлы сохраняются в `/root/wifi-monitor-backups/<дата-время>-<pid>/original/` с исходными путями и правами. Замена начинается только после подготовки всех файлов и резервных копий. При неудачной первой установке новые файлы компонента удаляются. Если неисправность файловой системы мешает и восстановлению, потребуется восстановить файлы вручную по напечатанному пути резервной копии.
...
Установку, обновление с резервной копией, удаление и восстановление после сбоев можно проверить на OpenWrt или Linux командой `sh tests/install.sh`. Проверки используют временные деревья через `DESTDIR`, искусственные ошибки и подменённые загрузки; службы не перезапускаются. Проверяются запуск из старой папки, ошибки подготовки и замены файлов, неудачная первая установка, ошибки скачивания установщика и компонентов. Тестовые папки сохраняются для осмотра.

Версия: **1.1.0**. См. [описание релиза](docs/releases/v1.1.0.md).
